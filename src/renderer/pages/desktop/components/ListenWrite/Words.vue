  ...
  <full-parent v-loading="loading" style="overflow-x: hidden;background-color: white">
    ...
    <div class="word-area">
      ...
      >
        ...
        <div>
          美式音标：[{{ visualTsInfo.basic['us-phonetic'] }}]<br>
          英式音标：[{{ visualTsInfo.basic['uk-phonetic'] }}]
        </div>

        <div class="title">词义</div>
        <div>
          <div v-for="(explain,index) in visualTsInfo.basic.explains" :key="index"
               style="margin: 10px 0">
            {{ explain }}
          </div>
        </div>

        <div v-if="visualTsInfo.web">
          <div class="title">相关</div>
          <div v-for="(word,index) in visualTsInfo.web" :key="index" style="margin: 10px 0">
        ...
        <el-form>
          ...
          <el-form-item label="听词得分">
            <label style="color: #409EFF">{{
                hearCount !== 0 ? parseInt((hearCount - errorCount) / hearCount *
                    100) : 0
              }}</label>（<label
